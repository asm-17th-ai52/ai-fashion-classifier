...
log = structlog.get_logger("vision.step0")
...
def node_validate_image(state: VisionState) -> dict:
    ...
    if error:
        log.warning("vision_validate", valid=False, reason=error, latency_ms=elapsed_ms)
    else:
        log.info("vision_validate", valid=True, latency_ms=elapsed_ms)
