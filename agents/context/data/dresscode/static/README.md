...
source: hand_curated                          # 'hand_curated' | 'tier2_promoted' (PR-D 부터)
...
- **색상**: `agents/vision/tools/color_lookup.py::_COLOR_TABLE` 의 한글 색상명만 사용.
  ...
  결정적으로 비교할 수 있다.
- **카테고리**: 위 스키마 표의 한글 카테고리 (Vision Agent 실측 출력 기준, 2026-05-10 확인).
  영어 카테고리 (`shirt`, `slacks`, …) 를 절대 섞어 쓰지 않는다.

## 업데이트 정책

| 단계 | 절차 |
|---|---|
| **현재 (PR-A)** | `hand_curated` — 본 PR 에서 사람이 직접 작성. spec §5.1 의 schema 만 만족하면 됨. |
| **PR-D 이후** | `tier2_promoted` — Tier-2 ReAct 가 생성한 임시 문서가 승격(promotion) 큐에 적재됨. 팀원 검수(주 1회) 후 PR 로 본 디렉터리에 편입. spec §8 참조. |
...
빌드 스크립트는 PR-B 에서 추가된다.

```bash
# PR-B 이후 가능
python -m agents.context.scripts.build_faiss_index
```
